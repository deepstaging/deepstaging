...
public sealed record Template(TemplateName Name, object? Context = null)
{
    ...
    private static readonly ConcurrentDictionary<string, Scriban.Template> ParsedTemplateCache = new();
    ...
    private static RenderResult Render(TemplateName name, string text, object? context = null)
    {
        try
        {
            ...
            var scribanTemplate = ParsedTemplateCache.GetOrAdd(text, t => Scriban.Template.Parse(t));
    ...
    /// </summary>
    private static RenderResult CreateParseErrorResult(TemplateName name, Scriban.Template template)
    {
        var diagnostic = Diagnostic.Create(
            Rules.TemplateRenderError,
            null, "ParsingError",
