...
using Deepstaging.Projection.Effects.Models;

namespace Deepstaging.Testing.Projection.Models;
...
public sealed record TestRuntimeModel
{
    ...
    /// <summary>
    /// The name of the generated composite capabilities interface from the production runtime (e.g., "IRuntimeCapabilities").
    /// </summary>
    public string CapabilitiesInterfaceName => $"I{RuntimeType.Name}Capabilities";

    /// <summary>
    /// The fully qualified capabilities interface name including namespace.
    /// </summary>
    public string CapabilitiesInterfaceFullName =>
        string.IsNullOrEmpty(RuntimeType.Namespace)
            ? CapabilitiesInterfaceName
            : $"global::{RuntimeType.Namespace}.{CapabilitiesInterfaceName}";
}
