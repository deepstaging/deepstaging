...
namespace Deepstaging.Projection.Ids.Attributes;

/// <summary>
/// Query wrapper for <see cref="TypedIdAttribute"/> data.
/// </summary>
public sealed record TypedIdAttributeQuery(AttributeData AttributeData) : AttributeQuery(AttributeData)
{
    /// <summary>
    /// Gets the backing type for the strongly-typed ID.
    /// </summary>
    public BackingType BackingType =>
        NamedArg<int>(nameof(TypedIdAttribute.BackingType))
            .ToEnum<BackingType>()
    ...
    public ValidSymbol<INamedTypeSymbol> BackingTypeSymbol(SemanticModel model)
    {
        return BackingType switch
        {
            ...
            _ => throw new ArgumentOutOfRangeException(paramName: nameof(BackingType), BackingType, null)
        };
    }
    ...
    public IdConverters Converters =>
        NamedArg<int>(nameof(TypedIdAttribute.Converters))
            .ToEnum<IdConverters>()
