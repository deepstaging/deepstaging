...
public static class ActivityEffectExtensions
{
        ...
        IEnumerable<KeyValuePair<string, object?>>? tags = null)
    {
        return Eff<RT, A>.LiftIO(async rt =>
        {
            ...
            try
            {
                ...
                LogSuccess(resolvedLogger, logLevel, activityName, stopwatch.ElapsedMilliseconds);
            ...
            catch (Exception ex)
            {
                ...
                LogError(resolvedLogger, ex, activityName, stopwatch.ElapsedMilliseconds);
    ...
    private static ILogger? ResolveLogger<RT>(
        RT rt,
        ILogger? explicitLogger,
        string activityName,
        ActivitySource? activitySource) =>
        explicitLogger ?? (rt as IHasLoggerFactory)?.LoggerFactory?.CreateLogger(
            activitySource?.Name ?? activityName);
